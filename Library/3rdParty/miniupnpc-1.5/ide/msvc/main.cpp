...
int main()
{
	...
	cIpAddr ip;
	bool ret = UpnpIgd().ExternalIp(&ip);

	//s32 num = UpnpIgd().NumberOfExistingPortMappings();
	ret = UpnpIgd().AddPortMapping(37999, 55001, cUpnpIgd::TCP, "My Redirect");
	ret = UpnpIgd().AddPortMapping(47999, 55002, cUpnpIgd::TCP, "My Redirect");
	//num = UpnpIgd().NumberOfExistingPortMappings();
