...
bool CMimeBody::ReadFromFile(const char* pszFilename)
{
	...
	//int hFile;
	//::_sopen_s(pszFilename, O_RDONLY | O_BINARY);
...
// JonB 
const CMimeMessage& CMimeMessage::operator= (const CMimeMessage& rhs)
{
	u32 size = rhs.GetLength();
	u8* buf = new u8[size];
	
	rhs.Store(reinterpret_cast<char*>(buf), size);

	Load(reinterpret_cast<char*>(buf), size);
...
void CMimeMessage::SetDate(int nYear, int nMonth, int nDay, int nHour, int nMinute, int nSecond)
{
	...
	struct tm ptmGmt;
	::gmtime_s(&ptmGmt, &timeDate);	// Greenwich Mean Time
	...
	::sprintf_s(szDate, 40, "%s, %d %s %d %02d:%02d:%02d %c%02d%02d",
